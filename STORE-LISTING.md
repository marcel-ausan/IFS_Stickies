...
Built by Marcel Ausan, IFS Cloud consultant. Available for setup, configuration and
custom work: https://www.linkedin.com/in/marcelausan
...
**Do not put LinkedIn here.** The dashboard refuses to publish with
`Support URL is not reachable`: LinkedIn redirects the trailing slash and then serves an
authentication wall to unauthenticated crawlers, so Google's checker never sees a page.
Verified 5 September 2026 — GitHub returns 200, LinkedIn does not.

The intent behind LinkedIn was right, though: the audience is IFS consultants and
administrators and the plan is services rather than a licence, so a route to a person is
worth having. It lives in the description instead, where nothing is fetched or checked.
